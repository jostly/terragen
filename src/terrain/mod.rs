...
impl Node {
    pub fn new(point: Vertex, elevation: f32) -> Node {
        Node { point: point, elevation: elevation }
    }
}
...
impl Terrain {
    ...
    pub fn current_level(&self) -> u8 { self.level }
    ...
    pub fn face_midpoint(&self, face: &Face) -> Point3<f32> {
    ...
    pub fn subdivide(&mut self) {
        ...
        for e in self.edges.iter() {
            ...
            let (midpoint, elevation) = {
                ...
                ((p0.point.coords + delta * 0.5).clone(), e + (random::<f32>() - 0.5)*self.rnd_pow)
            };

            let vidx = self.nodes.len() as u32;
            self.nodes.push(Node::new(Vertex::from_coordinates(normalize(&midpoint)), elevation));
        ...
        info!("After subdivision {} nodes, {} edges and {} faces", self.nodes.len(), self.edges.len(), self.faces.len());
    }
